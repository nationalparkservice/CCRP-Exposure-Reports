...
```{r, echo=FALSE}
fig1 <- paste0("data/", params$name, "/", params$name, "-Historical/Annual Means Lines Regressions Tmean Precip.png")
```

![**Figure 1.** Historical trends in annual average temperature (upper plot) and annual total precipitation (lower plot) for `r params$name` from 1895-2022. Black points show yearly values while the red line represents the 10-year rolling average. Blue lines are linear regressions for the entire period record, up to 1970, and 1970-present. Solid lines are significant trends (i.e., p\<0.05); dashed lines are not statistically significant.](%60r%20fig1%60){width="400"}
...
![**Figure 4.** Metrics of extreme temperature at `r params$name`. The upper plot represents the number of days per year when maximum temperature exceeds the historical 95th percentile at `r params$name` historically and in both climate futures. The lower plot is the number of days per year when heat index reaches 'dangerous' levels (103-124 °F).](%60r%20fig4%60){width="400"}
...
![**Figure 5.** Upper plot: Modeled recurrence intervals (annual) for 24-hour precipitation events for each climate future and the historical period. The x axis indicates the return period expected for the amount of precipitation received in a day indicated on the y axis (see Methodological note for details). Lower plot: Number of days per year in which rainfall ≥ `r Exposure.Data$HistPrecip99` inches, the historical 95th-percentile event.](%60r%20fig5%60){width="400"}
